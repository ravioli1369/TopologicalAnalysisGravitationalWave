...
class GWAnalyzer(object):
    ...
    def spectogram_features(self):
        ...
        def _postprocess_diagrams(
            ...
            min_values = [
                min_value if min_value != np.inf else 0 for min_value in min_values
            ]
    ...
    def obtain_topological_features(self, use_pointcloud: bool, use_spectogram: bool):
        if self.data is None:
            raise Exception("Preprocessor not run yet")
        ...
        if use_pointcloud:
            _features = np.hstack(
                (
                    _features,
                    self.point_cloud_features(1, 10, 1).reshape(self.data.shape[0], -1),
                )
            )
