...
class GWAnalyzer(object):
    ...
    def __init__(self, X):
        ...
        # Mutual infomration optimization needed
        #self.time_delay
        #self.dimension
        #self.stride
    ...
    def spectogram_features(self):
        ...
        def get_gudhi_persistence_diagram(arr):
            ...
            ph = complex_dict['complex'].persistence(homology_coeff_field = 2, min_persistence = 0)
            #ph_arr = [[b,d,dim] for dim,(b,d) in ph ]
            #ph_arr = np.array(ph_arr)
            #ph_arr[ph_arr == np.inf] = -np.min(arr)
            return ph
    ...
    def obtain_topological_features(self, use_pointcloud, use_spectogram):


        if self.data is None:
            raise Exception("Preprocessor not run yet")
            
        if use_pointcloud == False and use_spectogram == False:
    ...
    def save_features(self, path, name):
        ...
        if self.save_spectrograms:
            with open(f'{os.path.join(path, name)}_spectrograms.npy', 'wb') as f:
